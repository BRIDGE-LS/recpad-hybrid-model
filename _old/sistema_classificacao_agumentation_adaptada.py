...
class TMEDataset(Dataset):
    ...
    #from .augmentacoes_por_classe import get_augmentation_by_class
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            transform = get_augmentation_by_class(class_name)
...
def get_augmentation_by_class(class_name: str):
    transform_map = {
        "ADI": A.Compose([
            A.Resize(224, 224),
        ...
        "DEB": A.Compose([
            ...
            A.CoarseDropout(max_holes=3, max_height=20, max_width=20, fill_value=0, p=0.3),
            A.Normalize(mean=(0.485, 0.456, 0.406),
                        std=(0.229, 0.224, 0.225)),
            ToTensorV2()
        ]),
    ...
    return transform_map[class_name]
...
class DistributedTMETrainer:
    ...
    def train_model(self) -> Dict[str, any]:
        ...
        if self.rank == 0:
            checkpoint = torch.load('best_model_augmentation.pth')
            if self.is_distributed:
...
class TMETrainer:
    ...
    def _train_distributed(self) -> Dict[str, any]:
        ...
        if os.path.exists('best_model_augmentation.pth'):
            checkpoint = torch.load('best_model_augmentation.pth', map_location='cpu')
            return {
...
def create_validation_pipeline() -> Dict[str, any]:
    ...
    with open('validation_results_augmentation.json', 'w') as f:
        json.dump(final_results, f, indent=2, default=str)
    
    print(f"\n{'='*80}")
    print("PIPELINE CONCLUÍDO - Resultados salvos em 'validation_results_agumentation.json'")
    print(f"{'='*80}")
...
def run_hybrid_system_if_approved(baseline_results: Dict[str, any], config: TMEConfig) -> Optional[Dict[str, any]]:
    ...
    try:
        ...
        hybrid_results = hybrid_module.main_hybrid_pipeline("best_model_augmentation.pth")
...
def print_usage_instructions():
    ...
    print("   - Sistema salva automaticamente o melhor modelo em 'best_model_augmentation.pth'")
...
if __name__ == "__main__":
    ...
    try:
        ...
        #         print("📊 Verifique 'hybrid_validation_results_agumentation.json' para comparação")
        
        print("\n✅ PIPELINE CONCLUÍDO COM SUCESSO")
        print("📊 Resultados salvos em 'validation_results_agumentation.json'")
